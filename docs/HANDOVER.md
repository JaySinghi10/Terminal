...
- The old Project doc CONTEXT.md (8 September) is out of date in many places. This doc replaces it.
...
- For the app: `tsc` clean; eslint counts compared against the files' previous counts (several files already have lint errors, so "no new problems" is the bar); run the node test tools in `FlightTrackerApp/tools/` that apply.
...
- **Server:** Python 3.12 (Dockerfile `FROM python:3.12-slim`) with FastAPI on Google Cloud Run, service `flight-tracker`, project `flight-tracker-496006`, region `asia-south1`, scaling 0 to 20 instances (maxScale 20), no min instances (cold starts happen). Deployed from source (Cloud Build). Deploy notes in DEPLOY.md (partly stale, see T16).
...
- **Website:** static site in `Terminal website/`, hosted on Vercel, deployed by CLI.
...
### 5.1 Workflow
- Dev client: `npx expo start --dev-client` from `FlightTrackerApp` (add `--clear` if Metro misbehaves; OneDrive reparse tags broke Metro on Windows). Phone and computer must be on the same Wi-Fi. Force-quitting the dev client breaks the connection; reopen from the dev launcher. Port conflict: `npx kill-port 8081`.
- `FlightTrackerApp/.env.local` points dev builds at an old tag URL (EXPO_PUBLIC_API_BASE, honoured only in `__DEV__`, `lib/saved.tsx:117-118`). Don't copy it to the Mac; delete it on Windows.
- `FlightTrackerApp/.env` holds `EXPO_PUBLIC_WATCH_SECRET`. It is git-ignored and `eas.json` has no env block, so an EAS cloud build does not get it unless it is set in EAS; earlier TestFlight builds shipped an empty secret (`lib/watch.ts:266-270`). Build 13 must supply it (T9).
- EAS builds run from the `FlightTrackerApp` folder (a command pasted with a label once failed). `eas.json` has development, preview and production profiles; production has `autoIncrement: true`; `appVersionSource` is `remote`, so EAS keeps the iOS build number and `app.json`'s `buildNumber "10"` is likely ignored. There is a production submit profile. [UNCONFIRMED: the exact command used for builds 11 and 12 is not recorded anywhere in the repo; production profile plus auto-submit is the likely route.] A build takes about 30+ minutes, then Apple processing; the tester updates in the TestFlight app. Apple's "ready" email may not arrive; check TestFlight directly.
...
- Deploy: `cd "Terminal website" && vercel --prod --yes` [UNCONFIRMED: not run].
...
- `send_gmail_tests.py` is untracked and in no ignore file; there is no `.gcloudignore`, and the Dockerfile copies the whole folder. A deploy from the main working tree uploads it and bakes it into the image; revision 00107's upload included it and `.claude/settings.local.json`. It reads its credentials from the environment rather than holding them. Always deploy from a clean worktree; add it to `.dockerignore` and a new `.gcloudignore` (T7); delete the old bundles and images that carried it (T8).
...
13. `FlightTrackerApp/.env.local` points dev builds at an old tag.
...
3. Copy the root .env and FlightTrackerApp/.env from the carried copy. Do not copy .env.local. Keep send_gmail_tests.py outside the repo or ignored. Keep the bezel PNG out of git. FlightTrackerApp/.env's watch secret must also go into EAS for build 13 (T9).
