...
**The client's background refresher is gone, and the landing sweep that sat beside it is not dead.** The auto-refresh path had been disabled by a constant since before the poller existed, so every line under it was unreachable, including the resume check whose cooldown could never advance. It has been removed outright rather than left behind its flag. The pending-leg retry that reported the wrong trigger name -- a mount-time pass calling itself the daily one -- now names which of the two woke it.

**The client landing sweep is live and load-bearing, and it is the thing in this codebase most likely to be mistaken for dead code.** It runs on the store's minute tick and asks the `/landing` endpoint whether a saved flight has come down. The poller asks FR24 the same question, which is why it looks redundant, and it is not: the poller writes its answer into a state object in Cloud Storage, and nothing carries that back onto the device. `setFlightLanding`, which only this sweep calls, is the sole writer of `landedUtc`, `landingCheck` and `landingSource` on a saved flight -- `touchSavedFlight` deliberately preserves all three across an ordinary refresh rather than reading them from the response, because that response is AeroDataBox's and has never heard of Flightradar24. Remove the sweep and `effectiveStatus`, `landedInstant` and `aeroDataBoxMayLand` lose their only source at once, which is a card that silently stops reporting arrivals at the airports the second provider was added for.
